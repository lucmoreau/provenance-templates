...
- **Details**:
    ...
    Another characteristic of this template is that it provides a "justification" for `spec1` by explicitly linking it to the derivation, the entity `e0`, and the previous specialisation `spec0` via the attributes `openprov:derivation`, `openprov:previousEntity`, and `openprov:specialization`, respectively. These attributes are not defined in the PROV recommendations: they are the properties `openprov:hadDerivation`, `openprov:hadPreviousEntity` and `openprov:hadSpecialization` of the [OpenProvenance vocabulary](https://openprovenance.org/ns/openprov), written without `had` as PROV-N writes `prov:type`.
