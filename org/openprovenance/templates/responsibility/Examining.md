...
- **Details**:
    ...
    Initially, there is an entity `e0` that activity `examining` uses. After this operation, there is an entity `e1` with a new attribute `eprops1` with value `evalues1`, which is determined by the activity performed by `instrument` under the control of `agent`.  Any other attributes of `e0` (which might have been defined in other templates) are expected to remain unchanged.

    Using multiple bindings for `eprops1`-`evalues1`, this activity can create multiple attribute-value pairs.  While the existence of an instrument or an agent is optional, they allow for responsibility for the new attribute to be ascribed. 
    ...
    - Likewise, a further Triangle3-AGA describes how the activity `examining` was associated with `instrument`, who is also responsible for `e1` and its new attribute `eprops1`.


    - Finally, a delegation relation expresses that `instrument` operated under the control of `agent`.


    The `instrument` and `agent` are optional: either can be specified, or both can be specified. This allows for rich descriptions of responsibility for `e1` to be provided.

    The template `Examining` covers many common situations, such as when an agent determines the weight of a box using a scale (the weight would become a new attribute for the box), when an officer checks that an administrative request is complete (the flag complete would become an attribute for the request), or when an automous robotic arm checks that a parcel is damaged (the flag damaged would become an attribute for the parcel). These three examples illustrate configurations with an instrument-only (robotic arm), a human-agent-only (officer), and a combination of both (scale and agent).
