...
- **Details**:
   ...
   This template has two inputs, `input1`, `input2`; two outputs, `output1`, `output2`; an activity, `transforming`; an agent and a plan. This template is a direct instantiation of `org.openprovenance.templates.generic.Transformation1` that uses multiple values for `input` and `output`.
   ...
   On the contrary, `Parallel2` linked the input with the output variables, implying that each output is derived from a single input.
